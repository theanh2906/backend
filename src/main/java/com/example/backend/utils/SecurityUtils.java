...
public class SecurityUtils {
    public static UserDto getCurrentUser() {
        if (isAnonymous()) {
            return UserMapper.toModel(UserDetailsImpl.build(Constant.ADMIN));
        }
        return UserMapper.toModel((UserDetailsImpl) SecurityContextHolder.getContext().getAuthentication().getPrincipal());
    }
